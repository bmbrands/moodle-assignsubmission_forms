 ...
 *
 * @module     assignsubmission_forms/repository
...
/**
 * The Repository class provides methods to interact with the forms repository.
...
export default class Repository {
    ...
    static storeDraft(data) {
        return Ajax.call([{
            methodname: 'assignsubmission_forms_store_draft',
            args: data
        }])[0].then((response) => {
            if (response.error) {
                Notification.exception(response.error);
